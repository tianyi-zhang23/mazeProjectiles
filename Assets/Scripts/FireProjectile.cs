...
public class FireProjectile : MonoBehaviour
{
    ...
    void Update()
    {
        if(Input.GetMouseButtonDown(0) && projectileSourceController.GetComponent<ProjectileSourceCtrl>().hasProjectileSource()) //on left mouse click
        {
            ...
            Vector3 forceToAdd = Vector3.Scale(new Vector3(forceScale, forceScale, forceScale), Vector3.forward); //find the force vector scaled up according to the value specified
